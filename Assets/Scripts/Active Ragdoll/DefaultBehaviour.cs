...
public class DefaultBehaviour : MonoBehaviour {
    ...
    private void Update() {
        ...
        // When calling MovementInput, include sprinting state
        _activeRagdoll.Input.OnMoveDelegates += (movement) => MovementInput(movement, _isSprinting);
   ...
   private void UpdateMovement() 
    {
      if (_movement == Vector2.zero || !_enableMovement) {
        ...
        if (_movement != Vector2.zero && !_audioSource.isPlaying)
        {
            _audioSource.clip = walkingClip;
            _audioSource.Play();
        }

        float speedMultiplier = _isSprinting ? sprintMultiplier : 1.0f;
    _animationModule.Animator.SetBool("moving", true);
    _animationModule.Animator.SetFloat("speed", _movement.magnitude * speedMultiplier);        
    ...
    private void MovementInput(Vector2 movement, bool isSprinting)
    {
        _movement = movement;
    ...
    {
        MovementInput(movement, false); // Assume not sprinting by default
    }
