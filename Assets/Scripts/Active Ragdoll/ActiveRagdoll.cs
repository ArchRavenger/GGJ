...
namespace ActiveRagdoll {
    ...
    public class ActiveRagdoll : MonoBehaviour {
        ...
        public float impactForceMultiplier = 1000.0f;
        ...
        public void AdjustSpeed(float speedFactor)
        {
            ...
            {
                bodyPart.SetStrengthScale(speedFactor);
            }
        }
        void OnCollisionEnter(Collision collision) {
    if (collision.gameObject.CompareTag("Cannonball")) {
        Vector3 forceDirection = collision.impulse.normalized;
        float forceMagnitude = collision.impulse.magnitude * impactForceMultiplier;
          _physicalTorso.AddForce(forceDirection * forceMagnitude, ForceMode.Impulse);
    }
}
    }
} // namespace ActiveRagdoll